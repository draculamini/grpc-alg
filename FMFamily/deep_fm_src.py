...
class model():
    def __init__(self, args):
        ...
        self.embedding_size = args.embedding_size  # FM 公式里的 k
    ...
    def build_model(self):
        ...
        # 一次项中的w系数，类似原论文中的w  Yfm = wx + ∑∑ <vi, vj> xi*xj
        ...
        # self.weight["layer_0"] = tf.Variable(
        #     tf.random_normal([input_size, self.deep_layers[0]], 0.0, 0.01),
        #     name="layer_0"
        # )
        #
        ...
        self.weight["layer_0"] = tf.Variable(
            tf.random_normal([input_size, self.deep_layers[0]], 0.0, init_method), dtype=tf.float32, name="layer_0"
        )

        self.weight["bias_0"] = tf.Variable(
            tf.random_normal([1, self.deep_layers[0]], 0.0, init_method), dtype=tf.float32, name="bias_0"
        )
        ...
        self.embedding_index = tf.nn.embedding_lookup(self.weight['feature_weight'],
                                                      self.feat_index)  # (batch, feat_index.shape[-1], embedding_size)
        print(tf.shape(self.feat_index))
        print(tf.shape(self.embedding_index))
        ...
        # first_order FM第一部分 (W * X)
        self.embedding_first = tf.nn.embedding_lookup(self.weight['feature_first'],  # [self.feature_sizes, 1]
        ...
        # embedding_first (batch, feat_size, embeding_size)
        ...
        # FM 第二部分  核心部分  embedding_part = vx (∑i(v x))^2
